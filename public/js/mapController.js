...
var mapController = angular.module('mapController', ['ngFileUpload']);
mapController.directive('validFile',function(){
  return {
    require:'ngModel',
...
mapController.directive('ngFocusOut', function( $timeout ) {
...
mapController.controller('mapController', function($scope, $http, $q, Upload){
    ...
    searchBox.addListener('places_changed', function() {
      ...
      geocoder.geocode({ 'address': places[0]['formatted_address'] + ', Brasil', 'region': 'BR' }, function (results, status) {
        if (status == google.maps.GeocoderStatus.OK) {
            if (results[0]) {
                ...
                $scope.$apply(function(){
                    ...
                    $scope.formulario.latitude = latitude;

                    $scope.formulario.longitude = longitude;
                    $scope.formulario.comentario = "";

                    atualizar(latitude, longitude, true);
    ...
    $scope.validacao = function() {
        ...
        console.log('dentro da validacao');


        geocoder = new google.maps.Geocoder();
        geocoder.geocode({ 'address': $scope.formulario.endereco  + ', Brasil', 'region': 'BR' }, function (results, status) {
            if (status == google.maps.GeocoderStatus.OK) {
                if (results[0]) {
                    var latitude = results[0].geometry.location.lat();
                    var longitude = results[0].geometry.location.lng();
                ...
                $scope.$apply(function(){
                    $scope.formulario.latitude = "";
                    $scope.formulario.longitude = "";
                    $scope.formulario.comentario = "";
...
$scope.novoPonto = function() {

    function asyncGreet(file) {
  ...
  function savePoint(filename){
      ...
      var userData = {
       latlong: [$scope.formulario.longitude, $scope.formulario.latitude],
       comentario: $scope.formulario.comentario,
       endereco: $scope.formulario.endereco,
       file: filename
   ...
   $http({
    ...
    $scope.formulario.endereco = "";

    atualizar($scope.formulario.latitude, $scope.formulario.longitude);
...
asyncGreet($scope.formulario.file)
.then(savePoint).
catch(function(err){
    console.log(err);
});
...
var atualizar = function(latitude, longitude, autocomplete){
    ...
    $http({
        ...
        google.maps.event.addListener(polygon, 'click', function(e){
            ...
            $scope.$apply(function(){
               $scope.formulario.latitude = parseFloat(marker.getPosition().lat()).toFixed(3);

               $scope.formulario.longitude = parseFloat(marker.getPosition().lng()).toFixed(3);

               console.log(parseFloat(marker.getPosition().lat()).toFixed(3));

               geocoder = new google.maps.Geocoder();
               geocoder.geocode({ 'latLng': lastMarker.getPosition() }, function (results, status) {
                if (status == google.maps.GeocoderStatus.OK) {
                    if (results[0]) { 
                        console.log(results[0].formatted_address);
                        $scope.$apply(function(){

                          $scope.formulario.endereco = results[0].formatted_address;
                      });
...
$scope.carregarNoMapa = function() {
    ...
    if(typeof $scope.formulario.endereco !== 'undefined'){
        console.log($scope.formulario.endereco);
        geocoder = new google.maps.Geocoder();
        geocoder.geocode({ 'address': $scope.formulario.endereco + ', Brasil', 'region': 'BR' }, function (results, status) {
            if (status == google.maps.GeocoderStatus.OK) {
                if (results[0]) {
                    ...
                    $scope.$apply(function(){
                        ...
                        atualizar(parseFloat(latitude), parseFloat(longitude));
